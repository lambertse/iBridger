...
struct TestServer {
  ...
  explicit TestServer(const std::string& p, bool add_echo = false,
                      bool builtins = true)
      : path(p) {
...
TEST(ServerBuilder, MaxConnectionsPropagated) {
  ...
  auto server =
      ServerBuilder().set_endpoint(path).set_max_connections(4).build();
...
TEST(ClientStub, CallWhenDisconnectedReturnsError) {
  ...
  ibridger::Ping req;
  auto [resp, err] =
      stub.call<ibridger::Ping, ibridger::Pong>("ibridger.Ping", "Ping", req);
  EXPECT_TRUE(err);
}
...
TEST(ClientStub, TypedCallRoundtrip) {
  ...
  auto [pong, err] =
      stub.call<ibridger::Ping, ibridger::Pong>("EchoService", "Echo", req);
...
TEST(ClientStub, NotFoundMapsToErrc) {
  ...
  ibridger::Ping req;
  auto [resp, err] =
      stub.call<ibridger::Ping, ibridger::Pong>("ghost.Service", "Nope", req);
