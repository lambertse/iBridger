...
UnixSocketConnection::~UnixSocketConnection() { close(); }

std::pair<size_t, std::error_code> UnixSocketConnection::send(
    const uint8_t* data, size_t len) {
  size_t total = 0;
  while (total < len) {
    // MSG_NOSIGNAL suppresses SIGPIPE on Linux when the peer has closed.
    // On macOS SO_NOSIGPIPE is set at socket creation time (see
    // connect/accept).
#ifdef MSG_NOSIGNAL
...
std::pair<size_t, std::error_code> UnixSocketConnection::recv(uint8_t* buf,
                                                              size_t len) {
  while (true) {
...
bool UnixSocketConnection::is_connected() const { return connected_; }

ConnectionId UnixSocketConnection::id() const { return id_; }

// ─── UnixSocketTransport
// ──────────────────────────────────────────────────────

UnixSocketTransport::UnixSocketTransport() : server_fd_(-1), next_id_(1) {}
...
UnixSocketTransport::connect(const std::string& endpoint) {
  ...
  if (::connect(fd, reinterpret_cast<struct sockaddr*>(&addr), sizeof(addr)) <
    ...
    ::close(fd);
    common::Logger::error(
        "UnixSocket: connect failed on " + endpoint + ": " +
