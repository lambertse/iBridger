...
std::pair<size_t, std::error_code> NamedPipeConnection::recv(uint8_t* buf,
  ...
  // Return (0, {}) on broken pipe (ERROR_BROKEN_PIPE /
...
void NamedPipeConnection::close() {
  ...
  // if (connected_.exchange(false)) { CloseHandle(handle_); handle_ =
...
std::error_code NamedPipeTransport::listen(const std::string& endpoint) {
  ...
  // 2. CreateNamedPipe(endpoint, PIPE_ACCESS_DUPLEX, PIPE_TYPE_BYTE |
...
NamedPipeTransport::accept() {
  ...
  // 1. ConnectNamedPipe(server_handle_, nullptr) — blocks until a client
  // connects.
  // 2. On success, wrap server_handle_ in NamedPipeConnection and create a
  // fresh
